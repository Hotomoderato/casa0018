...
def countMargin(v, minSum, direction=True):
    '''
    将向量v中的数和minSum作比较，#目前返回向量v中比minSum小的数的数量或者返回的是索引
    Args:
        v = list
        minSum = Int
    Return:
...
def getLine(im, data, upperbound=8, lowerbound=25, threshold=30, h=40, minHeight=35, maxHeight=120, beginX=20, endX=-20,
            beginY=140, endY=1100, merged=True):
    ...
    for i in range(beginY, endY):
        ...
        if flag:
            ...
            if cnt >= threshold:
                ...
                if not merged or len(seg) == 0 or lineNo - seg[-1] - countMargin(dataSum[seg[-1]:], 5,
                                                                                 True) - countMargin(dataSum[:lineNo],
                                                                                                     5,
                                                                                                     False) > minHeight:
                    seg.append(lineNo)
                else:
    ...
    if not merged or len(seg) == 0 or lineNo - seg[-1] - countMargin(dataSum[seg[-1]:], 10, True) - countMargin(
            dataSum[:lineNo], 10, False) > minHeight:
        seg.append(lineNo)
    else:
        avg1 = dataSum[max(0, seg[-1] - 1):seg[-1] + 2]
        avg1 = avg1.sum() / avg1.shape[0]
    ...
    for i in range(0, len(seg) - 1):
        results.append(im.crop((0, seg[i] + countMargin(dataSum[seg[i]:], 0), im.size[0],
                                seg[i + 1] - countMargin(dataSum[:seg[i + 1]], 0, False))))
    return results
...
for i in range(len(imgFiles)):
    fin = open(txtFiles[i], errors='ignore')
    line = fin.readlines()
    # print(line)
    # line=line[0]
    fin.close()
...
h_conv2 = tf.nn.relu(
    tf.nn.conv2d(tf.nn.dropout(h_pool1, keep_prob), W_conv2, strides=[1, 1, 1, 1], padding='SAME') + b_conv2)
...
h_conv3 = tf.nn.relu(
    tf.nn.conv2d(tf.nn.dropout(h_pool2, keep_prob), W_conv3, strides=[1, 1, 1, 1], padding='SAME') + b_conv3)
...
h_conv4 = tf.nn.relu(
    tf.nn.conv2d(tf.nn.dropout(h_pool3, keep_prob), W_conv4, strides=[1, 1, 1, 1], padding='SAME') + b_conv4)
...
with sess:
    ...
    for ep in range(epoch):
        ...
        for i in range(0, Y.shape[0], batchSize):
            ...
            feed_dict = {inputs: X, seq_len: np.ones(min(Y.shape[0] - i, batchSize)) * 256, labels: batch_output,
                         keep_prob: 0.6}
            if ep < 50:
            ...
            print(ep, i, "loss:", tf.reduce_mean(loss.eval(feed_dict=feed_dict)).eval(), "err:",
                  tf.reduce_mean(width1_acc.eval(feed_dict=feed_dict)).eval())
